...
describe("email: real Resend + 11 templates", () => {
  it("9. sendEmail OTP_REGISTRATION performs a real send (id returned)", async () => {
    const r = await sendEmail({ to: "davidowi2@gmail.com", template: "OTP_REGISTRATION", vars: { name: "T", code: "123456" } });
    expect(r.ok).toBe(true);
    expect(r.id).toBeTruthy();
  });

  it("10. sendEmail PASSWORD_RESET renders resetLink", async () => {
    const r = await sendEmail({ to: "davidowi2@gmail.com", template: "PASSWORD_RESET", vars: { resetLink: "https://voeq.ng/reset?t=x" } });
    expect(r.ok).toBe(true);
  });
